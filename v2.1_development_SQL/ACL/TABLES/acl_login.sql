DROP TABLE IF EXISTS public.acl_login;
DROP SEQUENCE IF EXISTS public.acl_login_id_seq;

BEGIN;

	CREATE SEQUENCE IF NOT EXISTS public.acl_login_id_seq START 1;
	CREATE TABLE IF NOT EXISTS public.acl_login (
		id bigint DEFAULT nextval('acl_login_id_seq'::regclass) NOT NULL,
		usr character varying(100) DEFAULT NULL,
		eml character varying(100) DEFAULT NULL,
		pwd character varying(100) DEFAULT NULL,
		pwd_hash character varying(1000) DEFAULT NULL,
...
-- Insert batch #1
	INSERT INTO public.acl_login (id, eml, pwd, pwd_hash, token, active, timestamp, usr_id) VALUES
		...
		(47, 'usr_9@directtap.com', 'usr_9#1', NULL, NULL, 'True', '2017-06-13 12:37:37.292289', 50),
		(48, 'usr_10@directtap.com', 'usr_10#1', NULL, NULL, 'True', '2017-06-13 12:37:44.687171', 51),
